...
#[cfg(test)]
use {crate::moves::moves_utils::Move, std::collections::VecDeque, std::fs};
...
pub fn play_game(
    ...
    for (_, new_board) in legal_moves {
        n_moves += play_game(&new_board, move_gen_masks, hasher, depth + 1, max_depth);
    }
...
fn test_game(
    ...
    while let Some((current_depth, current_move, current_board, mut history)) = queue.pop_front() {
        ...
        for (new_move, new_board) in current_board.get_legal_moves(move_gen_masks, hasher) {
...
mod test_perft {
    ...
    fn test_position_1_default() {
        ...
        let max_depth = 6;
        let n_moves = play_game(&board, &MOVE_GEN_MASKS, &hasher, 1, max_depth);
    ...
    #[test]
    fn test_position_3() {
        let board = Board::from_fen("8/2p5/3p4/KP5r/1R3p1k/8/4P1P1/8 w - - 0 1", &HASHER).unwrap();
        let hasher = ZobristHasher::load();
